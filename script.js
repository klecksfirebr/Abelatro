...
async function gerarMao() {
    for (let i = 0; i < QuantMao; i++) {
        if (CartasExistentes.length < IdCartas.length - 1) {
            cardId = randomizarCarta(0, 51)
            console.log(CartasExistentes)
            if (CartasExistentes.includes(cardId)) {
                i = i - 1
            } else {
...
function ajeitarCarta() {
    const cartas = divCartas.querySelectorAll('.carta');
    const naipes = divCartas.querySelectorAll('.naipe');
    const numCartas = cartas.length;

    let larguraCarta = window.innerWidth * 0.07;

    const novaLargura = (window.innerWidth * 0.731) / numCartas;

    if (novaLargura < larguraCarta) {
        for (let i = 0; i < numCartas; i++) {
            cartas[i].style.width = novaLargura + 'px';
            naipes[i].style.width = novaLargura + 'px';
        }
    }
}
